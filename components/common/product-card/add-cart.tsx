import type { CardType, ProductCardProps } from '../types';
...
const ProductAddCart: React.FC<Partial<ProductCardProps & CardType & { addCartBtnClassName?: string }>> = props => {
  const {
    data,
    showAddBtn = true,
    addCartParams,
  ...
  const handleAddBtn = (type, num) => {
    if (
      data?.is_mkpl &&
      type === 'add' &&
      num === data?.min_order_quantity &&
      data?.vender_info_view?.free_threshold > 0
    ) {
  ...
  return (
    <div onClick={e => e.stopPropagation()}>
      {data?.category === 'bundle' || data?.is_hotdish === 'Y' ? (
        <div className={classNames('text-center absolute bottom-2 right-2 z-[1]')}>
          <span>{t('btn_view')}</span>
        </div>
      ) : showAddBtn ? (
        <div className={twMerge(classNames('text-center absolute bottom-2 right-2 z-[1]', addCartBtnClassName))}>
          <div
            onClick={e => {
              e.stopPropagation();
              e.preventDefault();
